...
%──────────────────────────────────────────────────────────────────────────────
\section{The \texorpdfstring{$d=12$}{d=12} Witness}
\label{sec:d12}
%──────────────────────────────────────────────────────────────────────────────

The conditional construction of \S\ref{sec:stark} predicts that the fiducial
coordinates generate a specific ray class field.  For $d=12$ we exhibit that field
as an explicit, computable object and verify its arithmetic by machine.  We call
this a \emph{witness} rather than a theorem, and the distinction is exact: we do
not assert the mixed-signature Stark conjecture, we exhibit the ray class field it
predicts and check it.  What is on the page is a constructed object whose defining
data and field identities a proof assistant confirms, not a claim resting on an
open hypothesis.  This is a concrete instance of the Hilbert-Twelfth construction
of \S\ref{sec:hilbert12}, realizing the base-field discriminant explicitly.
...
\subsection{A computable realization}

We have begun realizing $K_{12}$ as a computable object whose field identities
close by \texttt{native\_decide} in \Lean{}.  Two components are complete and
verified.  First, the degree-$32$ moduli subfield, built as a stack of computable
quadratic extensions $B[t]/(t^2 - r)$ with each generator validated against its
defining polynomial (that $i^2 = -1$, that each surd squares to its radicand, and
that each surd recovers $\sqrt{13}$).  Second, a flat degree-$288$ reduction engine
over $\mathbb{Q}$ (coefficient vectors of length $288$ under a single absolute monic
reduction rule) in which $\theta^{288}$ reduces correctly; the flat encoding avoids
the elaboration blow-up of a deeply nested tower.  Several fiducial coordinates lie
in subfields of $K_{12}$: $v_{12}(0)$ has degree $8$, $v_{12}(4)$ degree $16$, and
$v_{12}(2), v_{12}(3)$ degree $32$, each dividing $288$.  The crux coordinates do not
(\S\ref{subsec:doublecover}): $v_{12}(1)$ is irreducible of degree $64$, so the
coordinate field properly contains $K_{12}$.
...
$\varepsilon_{13} = (3+\sqrt{13})/2$ (regulator $1.19476\ldots$); pinning the exact
$S$-unit representative inside the fixed class is the one remaining conventional-form task.

\subsection{Scope}

This realizes the ray class field $K_{12}$ explicitly and confirms it is generated by the
fiducial \emph{phase and modulus} data --- an instance of the Hilbert-Twelfth construction
of \S\ref{sec:hilbert12} for $F_{12} = \mathbb{Q}(\sqrt{13})$ --- while the full coordinate
field is the ramified $S$-unit double cover $K_{12}(\sqrt{N_k})$ of \S\ref{subsec:doublecover}.
It is not a discharge of the existence axiom: pinning the exact $S$-unit, expressing all
twelve coordinates as exact elements of the double cover, and verifying the $143$
Weyl--Heisenberg overlap identities in the computable field remain in progress.  What is
established is the exact ray class field, its cyclic-piece decomposition, a verified
computable model of its arithmetic at full degree, and the double-cover structure of the
coordinate field over it.
