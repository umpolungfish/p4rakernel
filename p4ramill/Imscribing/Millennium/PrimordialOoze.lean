...
theorem phi_c_alone_is_O_1 :
    imscriptionTier ({ primordialOoze with pol := .P_asym } : Imscription) = .O_1 := by
  unfold primordialOoze imscriptionTier ouroboricityTier; decide
...
theorem P_pm_sym_alone_is_O_0 :
    imscriptionTier ({ primordialOoze with crit := .Phi_sub } : Imscription) = .O_0 := by
  unfold primordialOoze imscriptionTier ouroboricityTier; decide
...
theorem primordial_ooze_complete : (
    ...
    imscriptionTier ({ primordialOoze with pol := .P_asym } : Imscription) = .O_1 ∧
    imscriptionTier ({ primordialOoze with crit := .Phi_sub } : Imscription) = .O_0 ∧
