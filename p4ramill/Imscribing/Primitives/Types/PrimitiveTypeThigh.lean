...
/-- The fragment in ordinary mathematics, which is what a reader outside the
    Grammar reads. -/
def the_primitive_type_called_thigh_conventionalReading : String := "some member equinumerous with the whole: Dedekind-infinite."
...
def the_primitive_type_called_thigh_fragmentIsStatement : String := "never"

/-- The count behind that verdict, so its strength is visible. -/
def the_primitive_type_called_thigh_fragmentIsStatementEvidence : String := "starred in none of the 5 sampled theorem(s) that carry it"
