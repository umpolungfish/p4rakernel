...
/-- The CLINK L8 fragment for 'judge' (⊣ = 𐑡), as the organism-layer
    navigator emits it. Not authored here: the string is the navigator's. -/
def the_primitive_type_called_judge_clinkFragmentL8 : String := "graph(x) ∧ branch(x)"

/-- Its L8 proximity to the reference layer, and the atom it promotes. -/
def the_primitive_type_called_judge_proximityL8 : String := "distant"
def the_primitive_type_called_judge_promotedAtomL8 : String := ""

/-- The CLINK L9 fragment — the replicative-lateral reading of the same value.
    Where L9 annotates, the annotation is the transcendence content. -/
def the_primitive_type_called_judge_clinkFragmentL9 : String := "graph(x) ∧ branch(x)"
def the_primitive_type_called_judge_proximityL9 : String := "distant"
def the_primitive_type_called_judge_promotedAtomL9 : String := ""

/-- The fragment in ordinary mathematics, which is what a reader outside the
    Grammar reads. -/
def the_primitive_type_called_judge_conventionalReading : String := "a tree: connected and acyclic."
...
def the_primitive_type_called_judge_fragmentIsStatement : String := "never"

/-- The count behind that verdict, so its strength is visible. -/
def the_primitive_type_called_judge_fragmentIsStatementEvidence : String := "not carried by any of the seven sampled theorems"
