...
/-- The CLINK L8 fragment for 'ian' (≻ = 𐑾), as the organism-layer
    navigator emits it. Not authored here: the string is the navigator's. -/
def the_primitive_type_called_ian_clinkFragmentL8 : String := "lr⇔(x, y) ∧ Θ(x, y) ∧ ¬ Θ(y, x)"

/-- Its L8 proximity to the reference layer, and the atom it promotes. -/
def the_primitive_type_called_ian_proximityL8 : String := "match"
def the_primitive_type_called_ian_promotedAtomL8 : String := "LR_DUAL"

/-- The CLINK L9 fragment — the replicative-lateral reading of the same value.
    Where L9 annotates, the annotation is the transcendence content. -/
def the_primitive_type_called_ian_clinkFragmentL9 : String := "lr⇔(x, y) ∧ Θ(x, y) ∧ ¬ Θ(y, x)"
def the_primitive_type_called_ian_proximityL9 : String := "distant"
def the_primitive_type_called_ian_promotedAtomL9 : String := "LR_DUAL"

/-- The fragment in ordinary mathematics, which is what a reader outside the
    Grammar reads. -/
def the_primitive_type_called_ian_conventionalReading : String := "an ambidextrous adjunction — x is both left and right adjoint to y — carrying a strict order that holds one way only. Mutual determination with asymmetric precedence."
...
def the_primitive_type_called_ian_fragmentIsStatement : String := "never"

/-- The count behind that verdict, so its strength is visible. -/
def the_primitive_type_called_ian_fragmentIsStatementEvidence : String := "not carried by any of the seven sampled theorems"
